...
Description:
    This code uses a previously trained 2D UNet model to segment testing/inference data, the model was trained for
    Total Brains segmentation from cranial Ultrasound images of premature born infants.

Usage:
    python local_cpu_inference_2dUNet.py
...
def profile_model(model):
    macs, params = get_model_complexity_info(
        model,
        (1, local_inference_config.INPUT_IMAGE_WIDTH, local_inference_config.INPUT_IMAGE_HEIGHT),
...
for (patient, date), paths in grouped_paths.items():
    ...
    for path in paths:
        img = nib.load(path)
        image_arr = img.get_fdata().astype(np.float32)
        image_arr = resize(image_arr, (local_inference_config.INPUT_IMAGE_WIDTH, local_inference_config.INPUT_IMAGE_HEIGHT), order=3, anti_aliasing=True)

        image_arr = np.expand_dims(image_arr, axis=(0,1))  # shape: (1, 1, H, W)
